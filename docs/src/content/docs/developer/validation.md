...
- Render and key tests cover loading, empty, failure, truncation, minimum-width History layouts, Graph lanes and details, repository-search focus hints and results, file history and current content, message overlays, highlighted diff navigation, full-diff scrolling, comparison labels, default and configured bindings, leader sequences, semantic colors, and ambiguous-key rejection.
...
For this revision, the complete Rust gate above passed with 88 tests across unit, binary, CLI, and Git-service targets, followed by the locked release build. `cargo package --allow-dirty --locked` produced and rebuilt a 46-file crate (353.3 KiB, 71.0 KiB compressed). The documentation build generated 33 pages successfully.

The release binary was also exercised against this repository in a real 80×24 PTY. Graph loaded with visible parent lanes and `Enter` opened its changed-files/diff details as a centered floating window over the Graph. `q` returned to Graph. `Space f` opened repository file search; typing `R` updated Results live, `Ctrl-j` focused Results, and `Ctrl-k` restored Search with the `R` query intact. Appending `E` issued another live search and narrowed the list to README paths. `q` closed the active search prompt and uppercase `Q` exited successfully with the terminal restoration sequence. Fixed-text search, stale-result rejection, close/back behavior, and Ctrl-C quit are also covered by integration, reducer, executor, keymap, and rendering tests. The earlier History checks against coreutils remain recorded below.
