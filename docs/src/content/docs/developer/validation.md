...
- Reducer and effect tests cover every History panel, Graph details and back-navigation, repository-search prompts and selection, file-history/current-content transitions, full-message and full-content overlays, body-layout transitions/selection/scrolling, pagination, tree expansion, floating diff open/scroll/search/close transitions, navigation entered while a diff loads, message loading, stale-response rejection, and rapid diff coalescing.
- Render and key tests cover loading, empty, failure, truncation, minimum-width History layouts, Graph lanes and details, repository-search results, file history and current content, message overlays, highlighted diff navigation, full-diff scrolling, comparison labels, default and configured bindings, leader sequences, semantic colors, and ambiguous-key rejection.
- Temporary-repository integration tests cover root and merge commits, detached and unborn repositories, linked worktrees, staged-only and mixed changes, file and fixed-text search, current and historical file content, conflicts, renames, deletes, type changes, symlinks, submodules, binary and oversized diffs, and non-UTF-8 or leading-dash paths.
...
For this revision, the complete Rust gate above passed with 85 tests across unit, binary, CLI, and Git-service targets, followed by the locked release build. `cargo package --locked` from a clean tree produced and rebuilt a 46-file crate (342.6 KiB, 69.7 KiB compressed). The documentation build generated 33 pages successfully.

The release binary was also exercised against this repository in a real 80×24 PTY. Graph loaded with visible parent lanes; `m` opened the complete message; successive `Enter` presses opened the changed-files/diff details and then the full diff. `Space f`, a `README` query, and `Enter` opened File History with commit history above current content; moving the history selection changed the lower pane to that commit's diff. `q` exited successfully and the PTY session closed cleanly. Fixed-text search is covered by integration, reducer, keymap, and rendering tests. The earlier History checks against coreutils remain recorded below.
