...
:::note[配布状況]
`0.5.0`のmanifestはcrates.ioへ公開できる状態です。このversionをregistryから取得できるようになるまでは、信頼できるチェックアウトからインストールしてください。
:::
...
Windows、bareリポジトリ、パイプ、出力をキャプチャするコマンド、バックグラウンドセッションは`0.5.0`ではサポートされません。
...
`0.5.0`ではVim移動操作を拡充し、標準キーを変更しました。

- ビュー切替は`\1`から`\4`、リポジトリ検索は`\f` / `\g`を使います。リーダーは`\`になり、`Space`は文書カーソルを右へ移動します。
- 前後のペインへのフォーカス移動は`Ctrl-w h/k`と`Ctrl-w j/l`を使います。リポジトリ検索の結果からクエリ編集に戻る操作は`Ctrl-w k`です。
- 開いたテキスト文書内では、`Enter`は次行の最初の非空白文字へ移動します。すぐに閉じるには`q`を使います。DiffとCodeの標準`Esc`は、検索ハイライトがあれば先に消し、次の押下で閉じるか戻ります。

独自のキーマップは[キーマップリファレンス](/ja/reference/keymap/)に照らして確認してください。修飾キーのない`1`から`9`はカウント用に予約され、キーバインドの先頭には使えません。設定したアクションは標準キーを置き換えます。`close`を明示すると、`Esc`への割り当てでも即座に閉じます。
