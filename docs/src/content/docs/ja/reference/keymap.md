...
文書内検索のBackspaceは、通常時のカスタムキー設定にかかわらず、文字削除または既に空なら入力キャンセルとして予約されます。最後の文字を消した直後は再入力のためpromptを残します。Escによる即時キャンセルと、空Enterによる前回の語の再利用も維持します。リポジトリ検索のBackspaceは引き続きlive queryを編集します。
